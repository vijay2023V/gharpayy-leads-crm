...
import { CheckCircle2, AlertCircle, User, Phone, Mail, MapPin, Wallet, Sparkles } from "lucide-react";
...
export function DirectLeadForm() {
  ...
  const submit = () => {
    ...
    addLead({
      ...
      // Direct entry lives in the core store and persists through localStorage.
      source: "Direct",
      status,
    });
  ...
  return (
    <div className="space-y-4 rounded-xl border border-border bg-card p-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-semibold text-sm">Direct Lead Entry</h3>
          <p className="text-[11px] text-muted-foreground">Structured entry with required validation and live persistence.</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-end">
        <Button onClick={submit} className="h-9 gap-2">
          <CheckCircle2 className="h-4 w-4" /> Add lead
        </Button>
      </div>
    </div>
  );
